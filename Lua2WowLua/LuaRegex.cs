...
namespace Lua2WowLua
{
    public class LuaRegex
    {
        ...
        public static Regex SeeallModule = new Regex(@"^\s*module\s*\(" + NameSubexpression + @",\s*package\s*\.\s*seeall\s*\)\s*(;?\s*)*$");
        public static Regex UnhandledModule = new Regex(@"^\s*module\s*\(");
        public static Regex Require = new Regex(@"^\s*require\s*\(" + NameSubexpression + @"\)\s*(;?\s*)*$");
